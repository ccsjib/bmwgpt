...
(async function() {
    ...
    async function fetchWithRetry(url, attempt = 1) {
        try {
            ...
            if (response.status === 429) {
                ...
                console.warn(`⛔ 429 Rate Limit hit on: ${url}`);
                console.warn(`   ⏳ Cooling down for ${RETRY_WAIT / 1000} seconds...`);
            ...
            if (!response.ok) {
                console.error(`   ❌ Server error ${response.status} on ${url}`);
                return null;
            }

            return response;
        } catch (err) {
            console.error(`   ⚠️ Network error on ${url}`, err);
            return null;
        }
    }
    ...
    while (currentListingUrl) {
        console.log(`\n📂 Processing Page ${pageCount}: ${currentListingUrl}`);
        ...
        // Find Next Page
        const nextBtn = Array.from(doc.querySelectorAll('a')).find(el => el.textContent.includes('▸'));
        
        if (nextBtn && nextBtn.href) {
            currentListingUrl = nextBtn.href;
            pageCount++;
            console.log(`   ➡️ Moving to page ${pageCount}...`);
